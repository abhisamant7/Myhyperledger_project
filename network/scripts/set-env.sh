...
usage() {
    echo ". set-env.sh   ORG_NAME   [PEER_NAME default=dfarmadmin-peer1]  [IDENTITY default=admin] [PORT_NUMBER_BASE default=7050] [ORDERER_ADDRESS default=orderer.dfarmadmin.com:7050] "
...
if [ -z $2 ];
then
    echo "Switching PEER_NAME for Org = dfarmadmin-peer1"
    PEER_NAME=dfarmadmin-peer1
else
    PEER_NAME=$2
fi
...
if [ -z $5 ]
then
    echo "Setting ORDERER_ADDRESS=orderer.dfarmadmin.com:7050"   
...
# Capitalize the first letter of Org name e.g., dfarmadmin => Dfarmadmin  dfarmretail => Dfarmretail
